...
try {
    ...
    // Email headers (védelem email header injection ellen)
    // CRLF (\r\n) karakterek eltávolítása az email címekből és névből
    ...
    $headers[] = 'Message-ID: <' . time() . '-admin-' . md5($safeEmail) . '@dozsa-apartman-szeged.hu>';
    ...
    if ($mailSent) {
        ...
        $confirmationHeaders = [];
        $confirmationHeaders[] = 'From: ' . $safeFromName . ' <' . $safeFromEmail . '>';
        ...
        $confirmationHeaders[] = 'Message-ID: <' . time() . '-' . md5($safeEmail) . '@dozsa-apartman-szeged.hu>';

        $confirmationSent = mail($safeEmail, $confirmationSubject, $confirmationBody, implode("\r\n", $confirmationHeaders));

        // Log confirmation email result
        error_log('Contact form - Confirmation email sent to ' . $safeEmail . ': ' . ($confirmationSent ? 'SUCCESS' : 'FAILED'));
